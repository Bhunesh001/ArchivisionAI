...
def generate_floor_plan(plot_width, plot_length, building_type,
    ...
    # ── Open / garden zone ────────────────────────────────────
    # The open area is already accounted for by reducing build_area.
    # We do not draw it as a physical room block.
    ...
    for f in range(floors):
        if f == 0:
            floor_rooms = rooms_placed + park_rooms
        else:
...
def create_project():
    ...
    open_pct    = 0.0 # Hardcoded to 0 as feature was removed
